...
## 18. Scrolling, and readable glass

**Scrolling is the browser's own.** Lenis, GSAP ScrollTrigger and the
scroll-linked hero parallax were tried and removed. Smoothed wheel scrolling
means the page keeps animating after the wheel stops, and on a trackpad that
reads as the page lagging behind your hand however it is tuned. The wheel now
moves the page exactly as far as the reader asked.

What stayed, because none of it touches the scroll position:

- `scroll-behavior: smooth` on `html`, so in-page links glide instead of
  jumping. Turned off under reduced motion.
- The section after the hero is a sheet with 34px rounded top corners, a light
  edge and an upward shadow, lapping 26px over the photograph. A static shape,
  not an effect — it is what removes the hard seam.
- Sections fade up once as they come into view, through IntersectionObserver.
  One transition per element, never re-run, never tied to scroll offset.
